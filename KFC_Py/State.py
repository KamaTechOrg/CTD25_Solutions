...
class State:
    ...
    def on_command(self, cmd: Command, cell2piece: Dict[Tuple[int, int], List[Piece]], my_color: str = "X"):
        ...
        if cmd.type == "move":
            ...
            pieces_at_src = cell2piece.get(src_cell)
            if not pieces_at_src or len(pieces_at_src) == 0:
                  logger.debug(f"No piece at source cell {src_cell}")
                  return self
            piece = pieces_at_src[0]  # נניח שיש רק חייל אחד בתא
            piece_id = getattr(piece, "id", None)


            if not self.moves.is_valid(src_cell, dst_cell, cell2piece, self.physics.is_need_clear_path(), my_color, piece_id= piece_id):
                logger.debug(f"Invalid move: {src_cell} → {dst_cell}")
                return self
