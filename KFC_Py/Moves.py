...
class Moves:
    ...
    def is_dst_cell_valid(self, dr, dc, dst_pieces = None, my_color = None, dst_has_piece: bool | None = None):
        print(f"== is_dst_cell_valid called ==")
        print(f"dr, dc: {dr}, {dc}")
        print(f"dst_pieces: {dst_pieces}")
        ...
        move_tag = self.moves[(dr, dc)]
        print(f"move_tag: '{move_tag}'")

        if move_tag == "":  # No tag = can both capture/non-capture
            print("Move tag is empty – allowed for both capture and non-capture")

            return True

        if move_tag == "capture":
            result = dst_pieces is not None and len([p for p in dst_pieces if p.id[1] != my_color]) > 0
            print(f"Move is capture – result: {result}")
            return result
        
        if move_tag == "non_capture":
            result = dst_pieces is None
            print(f"Move is non-capture – result: {result}")

            return result
        print("Invalid move tag – returning False")

        return False  # Invalid tag

    # def is_valid(self, src_cell, dst_cell, cell2piece, is_need_clear_path, my_color,piece_id=None):
    #     # Check board boundaries
    #     if not (0 <= dst_cell[0] < self.dims[0] and 0 <= dst_cell[1] < self.dims[1]):
    #         return False
    ...
    #     if not self.is_dst_cell_valid(dr, dc, cell2piece.get(dst_cell), my_color):
    #         return False
    #     if is_need_clear_path and piece_id and piece_id[0] == 'N':  
    #         return True
    ...
    def is_valid(self, src_cell, dst_cell, cell2piece, is_need_clear_path, my_color, piece_id=None):
       print(f"\n[VALIDATING MOVE]")
       print(f"From {src_cell} to {dst_cell}")
       print(f"piece_id = {piece_id}")
       print(f"piece at dst = {cell2piece.get(dst_cell)}")
       print(f"my color = {my_color}")
       print(f"is_need_clear_path = {is_need_clear_path}")

       if not (0 <= dst_cell[0] < self.dims[0] and 0 <= dst_cell[1] < self.dims[1]):
          print("🛑 Move out of bounds")
          return False
       if piece_id and piece_id.startswith("N"):
           target = cell2piece.get(dst_cell)
           if target:
              target_color = target[0].id[1]
              if target_color == my_color:
                  print("🛑 Knight cannot eat same color")
                  return False
           print("✅ Knight move allowed (ignores path)")
           return True

       dr, dc = dst_cell[0] - src_cell[0], dst_cell[1] - src_cell[1]

       if not self.is_dst_cell_valid(dr, dc, cell2piece.get(dst_cell), my_color):
          print("🛑 Destination cell invalid (maybe own piece?)")
          return False

       if is_need_clear_path and piece_id and piece_id[0] == 'N':
          print("✅ Knight move, no need to check path")
          return True
