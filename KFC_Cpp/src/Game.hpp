...
inline void Game::draw() {
	//this->board = this->clone_board();
	//for (auto& p : this->pieces) {
	//	p->draw_on_board(this->board, this->game_time_ms());
	//}

	//// overlay both players" cursors, but only log on change
	//if (this->kp1 && this->kp2) :
	//for player, kp, last in(
	//	(1, this->kp1, "last_cursor1"),
	//	(2, this->kp2, "last_cursor2"))
	//{
	//}
	//	r, c = kp.get_cursor()
	//	// draw rectangle
	//	y1 = r * this->board.cell_H_pix;
	//x1 = c * this->board.cell_W_pix
	//	y2 = y1 + this->board.cell_H_pix - 1;
	//x2 = x1 + this->board.cell_W_pix - 1
	//	color = (0, 255, 0) if player == 1 else (255, 0, 0)
	//	this->board.img.draw_rect(x1, y1, x2, y2, color)

}
inline void Game::show() { board.show(); }
...
inline Game create_game(std::string pieces_root,
	...
	fs::path root = fs::absolute(fs::path(pieces_root)) / "pieces";
